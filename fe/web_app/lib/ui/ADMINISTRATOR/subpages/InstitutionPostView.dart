...
class InstitutionPostView extends StatefulWidget {
  final BuildContext ctx;
  final FilterController fc;
  InstitutionPostView(this.ctx, this.fc);
  @override
  _InstitutionPostViewState createState() => _InstitutionPostViewState();
}

class _InstitutionPostViewState extends State<InstitutionPostView> {
  ...
  _loadPosts() async
  {
    PostAPIServices.getFilteredPostsAdminPage("", -1, PostTypeEnum.institutionPost).then((value){
      _postStreamController.add(value);
      _fullList = value;
    });
  }

  _deletePost(AppPost post)
  {
    ...
    return Container(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          return TilePostComponent(widget.ctx, posts[index], _deletePost, key: UniqueKey(),);
        }
      )
    );
  }
